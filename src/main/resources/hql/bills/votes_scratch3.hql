...
as SELECT
    ...
    year,
    month
FROM bills.senate_votes_flat_external3 WHERE vote = 'yea' GROUP BY
    vote_id,
    vote,
    date,
    year,
...
as SELECT
    ...
    year,
    month
FROM bills.senate_votes_flat_external3 WHERE vote = 'nay' GROUP BY
    vote_id,
    vote,
    date,
    year,
...
as SELECT
    ...
    year,
    month
FROM bills.senate_votes_flat_external3 WHERE vote = 'not_present' GROUP BY
    vote_id,
    vote,
    date,
    year,
...
as SELECT
    ...
    year,
    month
FROM bills.senate_votes_flat_external3 WHERE vote = 'not_voting' GROUP BY
    vote_id,
    vote,
    date,
    year,
...
FROM bills.view_senate_votes_by_bill_yea y FULL OUTER JOIN bills.view_senate_votes_by_bill_nay n 
     ON y.yea_vote_id = n.nay_vote_id AND y.yea_date = n.nay_date FULL OUTER JOIN bills.view_senate_votes_by_bill_present p
     ON n.nay_vote_id = p.present_vote_id AND n.nay_date = p.present_date FULL OUTER JOIN bills.view_senate_votes_by_bill_not_voting nv
     ON p.present_vote_id = nv.not_voting_vote_id AND p.present_date = nv.not_voting_date;

nohup hive -e 'INSERT INTO TABLE bills.senate_votes_by_bill_external3 PARTITION (year, month) SELECT * FROM bills.view_joined_senate_votes_by_bill;'
