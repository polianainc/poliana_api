...
@Service
public class DEMOContributionService {
    @Autowired
    private ContributionHadoopRepo contributionHadoopRepo;
    @Autowired
    private IndustryService industryService;
    @Autowired
    private LegislatorService politicanService;
    ...
    public int getMainTotal(String billId) {
        return contributionHadoopRepo.sumIndustryContributions(billId, "yea");
    }

    public int getMainTouched(String billId) {
        return contributionHadoopRepo.countDistinctIndustryContributions(billId, "yeas");
    }

    public int getOffTotal(String billId) {
        return contributionHadoopRepo.sumIndustryContributions(billId, "nay");
    }

    public int getOffTouched(String billId) {
        return contributionHadoopRepo.countDistinctIndustryContributions(billId, "nays");
    }
    ...
    public List<IndustryContributor> getYeasByTotalCount(String billId, int limit) {
        List<IndustryContrTotals> industries = contributionHadoopRepo.industryContributionTotals(billId);
    ...
    public List<IndustryContributor> getYeasByInfluenceMetric(String billId, int limit) {
        List<IndustryContrTotals> industries = contributionHadoopRepo.industryContributionTotals(billId);
        final int votingYea = contributionHadoopRepo.countDistinctIndustryContributions(billId, "yeas");
        final int votingNay = contributionHadoopRepo.countDistinctIndustryContributions(billId, "nays");
        Collections.sort(industries, new Comparator<IndustryContrTotals>() {
            @Override
            public int compare(IndustryContrTotals industry1, IndustryContrTotals industry2) {
    ...
    public List<Recipient> yeaRecipients(String billId, String industryId, int limit) {
        ...
        if (currYeaBill == billId)
            ...
            currYeaBill = billId;
            tmpRecipients = this.yeaRecipients = contributionHadoopRepo.topIndustryRecipients(billId, "yeas");
        }
    ...
    public List<Recipient> nayRecipients(String billId, String industryId, int limit) {
        ...
        if (currNayBill == billId)
            ...
            currNayBill = billId;
            tmpRecipients = this.nayRecipients = contributionHadoopRepo.topIndustryRecipients(billId, "nays");
        }
        return recipientsHelper(tmpRecipients, industryId, limit);
    }
